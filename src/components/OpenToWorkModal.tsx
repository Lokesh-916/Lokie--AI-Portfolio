...
const OpenToWorkModal: React.FC<OpenToWorkModalProps> = ({ open, onOpenChange }) => {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-background z-52 max-h-[85vh] overflow-auto rounded-2xl border-none p-4 py-6 shadow-xl sm:max-w-[85vw] md:max-w-[80vw] lg:max-w-[1000px] flex flex-row items-center gap-10">
        {/* Profile Image as a box */}
        <div className="w-56 h-56 bg-gray-100 shadow-lg overflow-hidden flex items-center justify-center rounded-2xl flex-shrink-0 -mt-6">
          <Image
            src={profile.src}
            alt="Profile Picture"
            width={220}
        ...
        <div className="flex-1 flex flex-col items-start text-left min-w-0">
          <DialogTitle className="text-3xl font-bold mb-1">{profile.name}</DialogTitle>
          <DialogDescription className="mb-2 text-muted-foreground">
            {profile.age} • {profile.location}
          </DialogDescription>
          <p className="mb-4 text-base text-foreground">{profile.description}</p>
